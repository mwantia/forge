...
func RenderPrompt(renderer infratemplate.TemplateRenderer, cfg *EventConfig, ev EventInfo) (string, error) {
	tmpl, err := renderer.Clone(
		infratemplate.WithJsonValue("payload", ev.payload),

		infratemplate.WithValue("event.id", cty.StringVal(cfg.ID)),
		infratemplate.WithValue("event.desc", cty.StringVal(cfg.Description)),
		infratemplate.WithValue("event.session", cty.StringVal(cfg.Session)),
		infratemplate.WithValue("event.branch", cty.StringVal(cfg.Branch)),
		infratemplate.WithValue("event.model", cty.StringVal(cfg.Model)),
		infratemplate.WithValue("event.fired_at", cty.StringVal(ev.firedAt.UTC().Format(time.RFC3339))),
		infratemplate.WithValue("event.http.method", cty.StringVal("POST")), // Currently fix
	)
	if err != nil {
...
func assembleUserMessage(renderer infratemplate.TemplateRenderer, entries []EventInfo, cfg *EventConfig) (string, error) {
	n := len(entries)
